...
function loadRows(filePath: string): any[] {
  ...
  if (json && Array.isArray(json.tech)) return json.tech;
  if (json && Array.isArray(json.rows)) return json.rows;
  throw new Error("Expected JSON array or { tech: [...] } / { rows: [...] }");
}
...
async function main() {
  ...
  if (!input) {
    throw new Error(
      "Usage: npx ts-node scripts/firestore_import_tech_index.ts <path-to-tech-index-json>"
    );
  }
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    for (const r of slice) {
      const idCandidate =
        ...
        "";
      const ref = idCandidate
        ? firestore.collection(COL).doc(idCandidate)
