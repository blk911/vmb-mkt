...
export default function UnknownResolverQueue({ rows, onUpdateDecision }: Props) {
  if (!rows.length) {
  ...
  return (
    <div className="space-y-2">
      {rows.map((row) => (
        <UnknownResolverQueueCard key={row.record.id} row={row} onUpdateDecision={onUpdateDecision} />
      ))}
    </div>
  );
}
