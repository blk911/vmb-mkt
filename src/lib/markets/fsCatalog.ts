...
/**
 * Prefer gray-pin resolved → full → anchor supplement → presence-merged → plain enriched → base.
 * (Used by `loadZoneMembers` in `markets.ts`.)
 */
export function resolveZoneMembersJsonPath(): string | null {
  const resolved = marketsJsonPath("beauty_zone_members_enriched_resolved.json");
